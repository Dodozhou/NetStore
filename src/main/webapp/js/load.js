...
$(document).ready(function () {
   ...
   $.getJSON("http://localhost:8888/product?method=findAll",function (ps) {
     $.each(ps,function(index){
        $("#page_hot").after(
            "<a href='http://localhost:8888/product?method=findById&id="+ps[index].id+"'>"+
            "<div class='col-6'>"+
            "<div class='shangpin1' >"+
    ...
    $.getJSON("http://localhost:8888/purchase?method=findAll12",function (pu) {
        $.each(pu,function(index){
            $("#purchase_show").after(
                "<p><a href='purchase_information.jsp'>"+
                pu[index].description+
                "</a></p>"
                );
        })
    });
